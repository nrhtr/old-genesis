...
void op_set_obj_var(void)
{
    ...
    result = object_assign_var(cur_frame->object, cur_frame->method->object, id, val);
    if (result == varnf_id)
...
void op_map(void)
{
    ...
    if (cnt)
        ...
        case OP_MAPHASH:
            ...
            data_dup(list_last(list->u.list->el[0].u.list), list_elem(returned->u.list, 0));
            list->u.list->el[1].u.list->len++;
            data_dup(list_last(list->u.list->el[1].u.list), list_elem(returned->u.list, 1));
        }
    ...
    if (cnt >= len) {
        ...
        case OP_MAPHASH:
            domain->type = DICT;
            domain->u.dict = dict_new(list->u.list->el[0].u.list, list->u.list->el[1].u.list);
...
void op_map_range(void)
{
    ...
    if (counter->type != INTEGER || top->type != INTEGER) {
        cthrow(type_id, "Range bounds (%D, %D) are not both integers.", counter, top);
        return;
    }
    ...
    if (counter->u.val > top->u.val) {
        ...
        case OP_MAPHASH_RANGE:
            counter->type = DICT;
            counter->u.dict = dict_new(list->u.list->el[0].u.list, list->u.list->el[1].u.list);
            list_discard(list->u.list->el[0].u.list);
            list_discard(list->u.list->el[1].u.list);
            break;
        }
...
void op_for_range(void)
{
    ...
    if (range[0].type != INTEGER || range[1].type != INTEGER) {
        cthrow(type_id, "Range bounds (%D, %D) are not both integers.", &range[0], &range[1]);
        return;
    }
...
void op_case_range(void)
{
    ...
    if (range[0].type != range[1].type) {
        cthrow(type_id, "%D and %D are not of the same type.", &range[0], &range[1]);
        return;
    } else if (range[0].type != INTEGER && range[0].type != STRING) {
        cthrow(type_id, "%D and %D are not integers or strings.", &range[0], &range[1]);
        return;
    }
...
void op_get_obj_var(void)
{
    ...
    ind = cur_frame->opcodes[cur_frame->pc++];
    id = object_get_ident(cur_frame->method->object, ind);
    result = object_retrieve_var(cur_frame->object, cur_frame->method->object, id, &val);
    if (result == varnf_id) {
        cthrow(varnf_id, "Object variable %I not found.", id);
    } else {
        check_stack(1);
...
void op_message(void)
{
    ...
    if (call_method(objnum, message, target - stack, arg_start, is_frob) == CALL_ERROR)
...
void op_buffer(void)
{
    ...
    for (i = 0; i < len; i++) {
        if (stack[start + i].type != INTEGER) {
            cthrow(type_id, "Element %d (%D) is not an integer.", i + 1, &stack[start + i]);
            return;
        }
    }
    buf = buffer_new(len);
...
#define _CHECK_LENGTH_NEW(len) {\
        i = ind->u.val - 1;\
        if (i < 0) {\
            /*write_log("Transformed negative index %d of list length %d into ", i+1, len);*/\
            i = len + i + 1;\
            /*write_log("%dth element in mem.\n", i);*/\
            if (i < 0) {\
                cthrow(range_id, "(negative): Index is outside range..", i);\
                return;\
            }\
        }\
        if (i > len - 1) {\
            cthrow(range_id, "Index (%d) is greater than length (%d)", i + 1, len);\
            return;\
        }\
    }
...
void op_index(void)
{
    ...
    case LIST:
        _CHECK_TYPE;
        _CHECK_LENGTH_NEW(list_length(d->u.list));
    ...
    case STRING:
        _CHECK_TYPE;
        _CHECK_LENGTH_NEW(string_length(d->u.str));
    ...
    case BUFFER:
        {
            ...
            n = d2->u.val;
            if (n < 0) {
                cthrow(range_id, "Multiplying buffer %D with negative number %D.", d1, d2);
                return;
            }
            s = buffer_new(0);
            while (n--)
    ...
    case SET_OBJ_VAR:
        {
            ...
            result = object_assign_var(cur_frame->object, cur_frame->method->object, id, &d);
            if (result == varnf_id)
                cthrow(varnf_id, "Object variable %I not found.", id);
            break;
        }
...
{
    if (stack[stack_pos - 1].type != LIST) {
        cthrow(type_id, "Attempting to scatter non-list (%D)", &stack[stack_pos - 1]);
        return;
    }
...
void op_bwand(void)
{
    ...
    /* Make sure we're ANDing two integers. */
    if (d1->type != INTEGER) {
...
void op_bwor(void)
{
    ...
    /* Make sure we're ORing two integers. */
    if (d1->type != INTEGER) {
        cthrow(type_id, "Left side (%D) is not an integer.", d1);
    } else if (d2->type != INTEGER) {
        cthrow(type_id, "Right side (%D) is not an integer.", d2);
...
void op_bwshr(void)
{
    ...
    /* Make sure we're SHIFTing two integers. */
    if (d1->type != INTEGER) {
        cthrow(type_id, "Left side (%D) is not an integer.", d1);
    } else if (d2->type != INTEGER) {
        cthrow(type_id, "Right side (%D) is not an integer.", d2);
