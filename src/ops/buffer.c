...
COLDC_FUNC(bufgraft)
{
    ...
    if (pos > buffer_len(b1) || pos < 0)
        THROW((range_id, "Position %D is outside of the range of the buffer.", &args[1])) b1 = buffer_dup(b1);
    b2 = buffer_dup(b2);
...
COLDC_FUNC(buf_replace)
{
    ...
    if (pos < 0) {
        ...
        THROW((range_id, "Position (%d) is greater than buffer length (%d).", pos + 1, buffer_len(_BUF(ARG1))));
    }
...
COLDC_FUNC(subbuf)
{
    ...
    if (start < 0) {
        ...
        THROW((range_id, "The subrange extends to %d, past the end of the buffer (%d).", start + len, blen));
    }
