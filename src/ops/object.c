...
COLDC_FUNC(add_var)
{
    ...
    if (result == varexists_id)
        THROW((varexists_id, "Object variable %I already exists.", args[0].u.symbol)) pop(1);
    push_int(1);
}
...
COLDC_FUNC(set_var)
{
    ...
    result = object_assign_var(cur_frame->object, cur_frame->method->object, args[0].u.symbol, &args[1]);
    if (result == varnf_id) {
        cthrow(varnf_id, "Object variable %I does not exist.", args[0].u.symbol);
    } else {
...
COLDC_FUNC(get_var)
{
    ...
    result = object_retrieve_var(cur_frame->object, cur_frame->method->object, args[0].u.symbol, &d);
    if (result == varnf_id) {
        cthrow(varnf_id, "Object variable %I does not exist.", args[0].u.symbol);
    } else {
        pop(1);
...
COLDC_FUNC(default_var)
{
    ...
    result = object_default_var(cur_frame->object, cur_frame->method->object, SYM1, &d);
    if (result == varnf_id) {
...
COLDC_FUNC(inherited_var)
{
    ...
    result = object_inherited_var(cur_frame->object, cur_frame->method->object, SYM1, &d);
    if (result == varnf_id) {
        cthrow(varnf_id, "Object variable %I does not exist.", SYM1);
    } else {
        pop(1);
...
COLDC_FUNC(clear_var)
{
    ...
    if (cur_frame->object != cur_frame->method->object) {
        result = object_delete_var(cur_frame->object, cur_frame->method->object, args[0].u.symbol);
    }
...
COLDC_FUNC(add_method)
{
    ...
    for (d = list_first(code); d; d = list_next(code, d)) {
        if (d->type != STRING) {
            cthrow(type_id, "Line %d (%D) is not a string.", d - list_first(code), d);
            return;
        }
    }
...
COLDC_FUNC(method_access)
{
    ...
    switch (access) {
    case MS_PUBLIC:
        push_symbol(public_id);
        break;
    case MS_PROTECTED:
        push_symbol(protected_id);
        break;
    case MS_PRIVATE:
        push_symbol(private_id);
        break;
    case MS_ROOT:
        push_symbol(root_id);
        break;
    case MS_DRIVER:
        push_symbol(driver_id);
        break;
    case MS_FROB:
        push_symbol(frob_id);
        break;
...
COLDC_FUNC(method_info)
{
    ...
    case MS_PUBLIC:
        list[4].u.symbol = ident_dup(public_id);
        break;
    case MS_PROTECTED:
        list[4].u.symbol = ident_dup(protected_id);
        break;
    case MS_PRIVATE:
        list[4].u.symbol = ident_dup(private_id);
        break;
    case MS_ROOT:
        list[4].u.symbol = ident_dup(root_id);
        break;
    case MS_DRIVER:
        list[4].u.symbol = ident_dup(driver_id);
        break;
    case MS_FROB:
        list[4].u.symbol = ident_dup(frob_id);
        break;
    default:
        list[4].type = INTEGER;
        list[4].u.val = 0;
...
COLDC_FUNC(find_next_method)
{
    ...
    /* Look for the method on the current object. */
    m = object_find_next_method(cur_frame->object->objnum, SYM1, OBJNUM2, FROB_YES);
    m2 = object_find_next_method(cur_frame->object->objnum, SYM1, OBJNUM2, FROB_NO);
    if (!m) {
        m = m2;
    } else if (m2) {
        if (object_has_ancestor(m2->object->objnum, m->object->objnum)) {
...
COLDC_FUNC(data)
{
    ...
    if (nargs) {
        ...
        for (i = 0; i < obj->vars.size; i++) {
            if (obj->vars.tab[i].name == INV_OBJNUM || obj->vars.tab[i].cclass != objnum)
                continue;
            key.type = SYMBOL;
...
COLDC_FUNC(del_objname)
{
    ...
    if (!object_del_objname(cur_frame->object)) {
        cthrow(namenf_id, "Object #%l does not have a name.", cur_frame->object->objnum);
        return;
    }
...
COLDC_FUNC(objname)
{
    ...
    if (cur_frame->object->objname == -1)
        cthrow(namenf_id, "No name is assigned to #%l.", cur_frame->object->objnum);
    else
