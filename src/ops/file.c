...
COLDC_FUNC(fopen)
{
    ...
    if (file != NULL) {
        cthrow(file_id, "A file (%s) is already open on this object.", file->path->s);
        return;
    }
...
COLDC_FUNC(file)
{
    ...
    GET_FILE_CONTROLLER(file) info = list_new(5);
...
COLDC_FUNC(files)
{
    ...
    while ((dent = readdir(dp)) != NULL) {
        if (strncmp(dent->d_name, ".", 1) == F_SUCCESS || strncmp(dent->d_name, "..", 2) == F_SUCCESS)
            continue;
...
COLDC_FUNC(fchmod)
{
    ...
    if (argc == 1) {
        GET_FILE_CONTROLLER(file) path = string_dup(file->path);
    } else {
        struct stat sbuf;
...
COLDC_FUNC(fremove)
{
    ...
    if (err != F_SUCCESS)
        THROW((file_id, strerror(GETERR())))pop(1);
    push_int(1);
}
...
COLDC_FUNC(fseek)
{
    ...
    GET_FILE_CONTROLLER(file) if (!file->f.readable || !file->f.writable)
        THROW((file_id, "File \"%s\" is not both readable and writable.", file->path->s)) if (SYM2 == SEEK_SET_id)
            whence = SEEK_SET;
            ...
            THROW((file_id, strerror(GETERR())))pop(2);
    push_int(1);
}
...
COLDC_FUNC(frename)
{
    ...
    if (args[0].type != STRING || !string_length(STR1)) {
        GET_FILE_CONTROLLER(file) from = string_dup(file->path);
...
COLDC_FUNC(fflush)
{
    ...
    GET_FILE_CONTROLLER(file) if (fflush(file->fp) == EOF) {
        cthrow(file_id, strerror(GETERR()));
        return;
    }
...
COLDC_FUNC(fread)
{
    ...
    GET_FILE_CONTROLLER(file) if (!file->f.readable) {
...
COLDC_FUNC(fwrite)
{
    ...
    GET_FILE_CONTROLLER(file) if (!file->f.writable) {
    ...
    if (file->f.binary) {
        ...
        count = fwrite(args[0].u.buffer->s, sizeof(unsigned char), args[0].u.buffer->len, file->fp);
        ...
        count = fwrite(args[0].u.str->s, sizeof(unsigned char), args[0].u.str->len, file->fp);
...
COLDC_FUNC(fstat)
{
    ...
    if (!argc) {
        GET_FILE_CONTROLLER(file) stat_file(file, &sbuf);
    } else {
...
COLDC_FUNC(execute)
{
    ...
    for (d = list_first(script_args), i = 0; d; d = list_next(script_args, d), i++) {
        if (d->type != STRING) {
            cthrow(type_id, "Execute argument %d (%D) is not a string.", i + 1, d);
            return;
        }
    }
    ...
    fSuccess = CreateProcess((LPTSTR) fname, (LPTSTR) argv,
                             ...
                             (LPSECURITY_ATTRIBUTES) NULL, (BOOL) TRUE,
                             (DWORD) 0, NULL, NULL, (LPSTARTUPINFO) & SI, (LPPROCESS_INFORMATION) & pi);
