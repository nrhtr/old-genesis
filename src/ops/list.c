...
COLDC_FUNC(listgraft)
{
    ...
    if (pos > list_length(l1) || pos < 0)
        THROW((range_id, "Position %D is outside of the range of the list.", &args[1])) l1 = list_dup(l1);
    l2 = list_dup(l2);
...
COLDC_FUNC(range)
{
    ...
    if (!func_init_1_or_2(&args, &argc, INTEGER, INTEGER))
        return;
    ...
    if (len < 1) {
        pop(argc);
        push_list(list_new(0));
        return;
    }

    new = list_new(end - start);
    Int x;
    for (x = start; x < end; x++) {
        item.u.val = x;
        new = list_add(new, &item);
    }
...
COLDC_FUNC(sublist)
{
    ...
    if (start < 0) {
        ...
        cthrow(range_id, "Sublist extends to %d, past end of list (length %d)", start + span, list_len);
    } else {
...
COLDC_FUNC(insert)
{
    ...
    if (pos < 0) {
        ...
        cthrow(range_id, "Position (%d) beyond end of list (length %d)", pos + 1, list_len);
    } else {
...
COLDC_FUNC(replace)
{
    ...
    if (pos < 0) {
        ...
        cthrow(range_id, "Position (%d) greater than length of list (%d)", pos + 1, list_len);
    } else {
        /* Modify the list and pop the offset and data. */
        anticipate_assignment();
...
COLDC_FUNC(listidx)
{
    ...
    if ((r = list_index(LIST1, &args[1], origin)) == F_FAILURE)
        THROW((range_id, "Origin is beyond the range of the list.")) pop(argc);
    push_int(r);
}
