...
static unsigned int addr_size = sizeof(sockin);  /* Size of sockin. */
...
INTERNAL int use_prebound(SOCKET * sock, int port, char *addr, int socktype)
{
    ...
    while (*pbp) {
        ...
        if (pb->port == port) {
            ...
            if ((pb->tcp && socktype == SOCK_DGRAM) || (!pb->tcp && socktype == SOCK_STREAM)) {
 ...
 * happened. */
Int io_event_wait(Int sec, Conn * connections, server_t * servers, pending_t * pendings)
{
    ...
    for (serv = servers; serv; serv = serv->next) {
        if (FD_ISSET(serv->server_socket, &read_fds)) {
            serv->client_socket = accept(serv->server_socket, (struct sockaddr *) &sockin, &addr_size);
    ...
    for (pend = pendings; pend; pend = pend->next) {
        if (FD_ISSET(pend->fd, &write_fds)) {
            result = getpeername(pend->fd, (struct sockaddr *) &sockin, &addr_size);
            if (result == SOCKET_ERROR) {
                getsockopt(pend->fd, SOL_SOCKET, SO_ERROR, (char *) &error, &dummy);
