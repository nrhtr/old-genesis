...
{
    cList *fields = (cList *) NULL, *elemlist;
    regexp *rx;
    cData d;
    Int i;
             ...
             Int flags, Int mult)
{
    ...
    char *s = string_chars(ss), /* start */
        ...
        *r;                     /* replace */
    register Int i, x;
    Int size = 1, slen = string_length(ss), rlen = string_length(rs);
    ...
    forever {
        ...
        case 'e':
            ...
            if (pad) {
                if (pad <= 3) {
                    string_discard(out);
                x_THROW((type_id, "Elipse pad length must be at least 4 or more."))}
...
cList *strsplit(cStr * str, cStr * reg, Int flags)
{
    ...
    if ((rx = string_regexp(reg)) == (regexp *) NULL)
        x_THROW((regexp_id, "%s", gen_regerror(NULL)))
            ...
            /* rrg */
            if (*s == '(')
                x_THROW((regexp_id, "split only supports simple regular expressions right now."));
        }
