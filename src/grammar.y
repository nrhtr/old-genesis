...
%type	<expr>		expr sexpr rexpr handler key_datum
%type	<expr_list>	args arglist cvals dict_args kd_list
...
%token		FORK
%token		PASS CRITLEFT CRITRIGHT PROPLEFT PROPRIGHT DICT_EQ
...
%left	'+' '-'
%left	'*' '/' '%'
%left   BW_OR BW_AND SHLEFT SHRIGHT
%left	'!'
...
%token F_LOWERCASE F_STRCMP F_STRMEMCMP F_INTERNED F_ID F_RANGE F_GLOB F_LISTLEN F_SUBLIST F_INSERT F_JOIN F_REPLACE
...
%token F_CREATE F_CHPARENTS F_DESTROY F_R_DBLOG F_DBLOG F_REASSIGN_CONNECTION
%token F_BACKUP F_BINARY_DUMP F_TEXT_DUMP F_EXECUTE F_SHUTDOWN F_HARD_EXIT_ERROR
...
expr	: INTEGER			{ $$ = integer_expr($1); }
	...
	| expr '&' expr			{ $$ = binary_expr('&', $1, $3); }
	| expr '|' expr			{ $$ = binary_expr('|', $1, $3); }
	| expr SHLEFT expr		{ $$ = binary_expr(SHLEFT, $1, $3); }
	| expr SHRIGHT expr		{ $$ = binary_expr(SHRIGHT, $1, $3); }
	...
	| map '(' expr ')' TO '(' expr ')'
					{ $$ = map_expr($3,$1,$7,OP_MAP); }
	| map '[' expr UPTO expr ']' TO '(' expr ')'
					{ $$ = map_range_expr($3,$5,$1,$9,OP_MAP_RANGE); }
	| hash '(' expr ')' TO '(' expr ')'
					{ $$ = map_expr($3,$1,$7,OP_MAPHASH); }
	| hash '[' expr UPTO expr ']' TO '(' expr ')'
					{ $$ = map_range_expr($3,$5,$1,$9,OP_MAPHASH_RANGE); }
	| find '(' expr ')' WHERE '(' expr ')'
					{ $$ = map_expr($3,$1,$7,OP_FIND); }
	| find '[' expr UPTO expr ']' WHERE '(' expr ')'
					{ $$ = map_range_expr($3,$5,$1,$9,OP_FIND_RANGE); }
...
kd_list : key_datum			{ $$ = expr_list($1, NULL); }
	| kd_list ',' key_datum		{ $$ = expr_list($3, $1); }
	;

key_datum : expr DICT_EQ expr		{ $$ = list_expr(expr_list($3, expr_list($1, NULL))); }
	  ;
