...
INTERNAL cStr *unparse_dict(cStr * str, Expr_list * args);
INTERNAL cStr *unparse_expr_prec(cStr * str, Expr * expr, Int caller_type, Int assoc);
INTERNAL cStr *unparse_complex_expr(cStr * str, Expr * expr, Int caller_type, Int assoc);
        ...
        case IF_ELSE:
            {
                ...
                if ((if_flags & COMPLEX_FLAG) ||
                    ...
                    (else_flags & SPANNING_IF_FLAG && else_flags & COMPLEX_IF_FLAG)) {
        ...
        case OPTIONAL_ASSIGN:
            {
                ...
                args = expr_list(opt_expr(rhs->expr->u.assign.lval->u.name, rhs->expr->u.assign.value), args);
                break;
            }
        case SCATTER_START:
            pos++;
...
static Expr_list *decompile_expressions_bounded(Int * pos_ptr, Int expr_end)
{
    ...
    while (expr_end == -1 || pos < expr_end) {
        ...
        case OPTIONAL_ASSIGN:
            {
                ...
                stack =
                    expr_list(opt_expr
                              (rhs->expr->u.assign.lval->u.name,
                               rhs->expr->u.assign.value), stack->next /*skip the get_var */ );
                break;
            }
        ...
        case START_ARGS:
            {
                ...
                case EXPR_CALL_METHOD:
                    stack->next->expr = expr_message_expr(stack->next->expr, stack->expr, args);
                    stack = stack->next;
                    pos++;
                    break;
        ...
        case OP_HANDLED_FROB:
            stack->next->next->expr = frob_expr(stack->next->next->expr, stack->next->expr, stack->expr);
        ...
        case LE:
        case OP_IN:
            stack->next->expr = binary_expr(the_opcodes[pos], stack->next->expr, stack->expr);
            stack = stack->next;
            pos++;
            break;
...
static cList *unparse_stmt(cList * output, Stmt * stmt, Int indent, Stmt * last)
{
    ...
    case IF_ELSE:
        {
            ...
            while (stmt->type == IF_ELSE) {
                ...
                if (stmt->u.if_.cond->type == NOOP) {
                    ...
                    str = string_add_chars(str, " {", 2);
                    output = add_and_discard_string(output, str);
                    output = unparse_stmt(output, stmt->u.if_.true, indent + the_increment, NULL);
                    str = string_of_char(' ', indent);
                    str = string_add_chars(str, "} ", 2);
                } else {
                    output = add_and_discard_string(output, str);
            ...
            if (stmt->type == NOOP) {
                ...
                str = string_addc(str, '{');
                output = add_and_discard_string(output, str);
                output = unparse_stmt(output, stmt, indent + the_increment, NULL);
                str = string_of_char(' ', indent);
                str = string_addc(str, '}');
                return add_and_discard_string(output, str);
            } else {
    ...
    case SWITCH:
        ...
        output = add_and_discard_string(output, str);
        output = unparse_cases(output, stmt->u.switch_.cases, indent + the_increment);
        str = string_of_char(' ', indent);
        str = string_addc(str, '}');
        return add_and_discard_string(output, str);
    ...
    case CATCH:
        {
            ...
            if ((stmt->u.ccatch.body != NULL &&
                ...
                (stmt->u.ccatch.handler != NULL && is_complex_type(stmt->u.ccatch.handler->type)))
                complex = 1;
            ...
            if (complex)
                str = string_add_chars(str, " {", 2);
            output = add_and_discard_string(output, str);
            output = unparse_stmt(output, stmt->u.ccatch.body, indent + the_increment, NULL);
            if (stmt->u.ccatch.handler) {
                ...
                output = add_and_discard_string(output, str);
                output = unparse_stmt(output, stmt->u.ccatch.handler, indent + the_increment, NULL);
            }
            if (complex) {
                str = string_of_char(' ', indent);
                str = string_addc(str, '}');
...
static cList *unparse_case(cList * output, Case_entry * case_entry, Int indent)
{
    ...
    output = add_and_discard_string(output, str);
    return unparse_stmt_list(output, case_entry->stmts, indent + the_increment);
}
...
{
    return (expr->type == FUNCTION_CALL && !expr->u.function.args && strcmp(expr->u.function.name, "this") == 0);
}
...
/* recursive....? whatever... too tired... */
Expr_list *rev(Expr_list * head, Expr_list * reversed_tail)
{
    if (head) {
        Expr_list *next = head->next;
...
/* man... i suck.. is there an easier way to do this?
 * dicts all use list expressions internally :S */
static cStr *unparse_dict(cStr * str, Expr_list * args)
{
    args = rev(args, NULL);

    Expr_list *iter;
    for (iter = args; iter; iter = iter->next) {
        str = unparse_expr(str, iter->expr->u.args->next->expr, PAREN_ASSIGN);
        str = string_add_chars(str, " => ", 4);
        str = unparse_expr(str, iter->expr->u.args->expr, PAREN_ASSIGN);
        if (iter->next)
            str = string_add_chars(str, ", ", 2);
    }
    return str;
 ...
 * should be 1. */
static cStr *unparse_expr_prec(cStr * str, Expr * expr, Int caller_type, Int assoc)
{
    Int caller_prec = prec_level(caller_type), type, prec;

    type = (expr->type == BINARY) ? expr->u.binary.opcode : (expr->type == UNARY) ? expr->u.unary.opcode : expr->type;
...
static cStr *unparse_complex_expr(cStr * str, Expr * expr, Int caller_type, Int assoc)
{
    if (expr->type < DATA_END || expr->type == FUNCTION_CALL || expr->type == VAR) {
        return unparse_expr(str, expr, PAREN_ASSIGN);
    } else {
        str = string_addc(str, '(');
        str = unparse_expr(str, expr, PAREN_ASSIGN);
