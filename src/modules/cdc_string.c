...
{
    Int len;
    INIT_1_ARG(STRING) len = string_length(STR1);
...
NATIVE_METHOD(substr)
{
    ...
    INIT_2_OR_3_ARGS(STRING, INTEGER, INTEGER) slen = string_length(STR1);
    start = INT2 - 1;
    ...
    if (start < 0) {
        ...
        THROW((range_id, "The substring extends to %d, past the end of the string (%d).", start + len, slen));
    }
    str = string_dup(STR1);
...
NATIVE_METHOD(explode)
{
    ...
    switch (ARG_COUNT) {
    case 3:
        want_blanks = (Bool) data_true(&args[2]);
    case 2:
        INIT_ARG2(STRING);
    ...
    case 1:
        ...
        break;
    default:
        THROW_NUM_ERROR(ARG_COUNT, "one to three");
    }
...
NATIVE_METHOD(strsub)
{
    ...
    INIT_3_ARGS(STRING, STRING, STRING) s = string_chars(STR1);
...
NATIVE_METHOD(match_pattern)
{
    ...
    INIT_2_ARGS(STRING, STRING) str = string_chars(STR1);
...
NATIVE_METHOD(match_regexp)
{
    ...
    switch (ARG_COUNT) {
    case 3:
        sensitive = (Bool) data_true(&args[2]);
    case 2:
        INIT_ARG2(STRING);
        INIT_ARG1(STRING);
        break;
...
NATIVE_METHOD(regexp)
{
    ...
    case 3:
        sensitive = (Bool) data_true(&args[2]);
    case 2:
        INIT_ARG2(STRING) INIT_ARG1(STRING) break;
    default:
        THROW_NUM_ERROR(ARG_COUNT, "two or three");
    }
...
{
    cStr *out;
    Int flags = RF_NONE, mult = 2;
    DEF_args;

    switch (ARG_COUNT) {
    case 5:
        CHECK_TYPE(4, INTEGER, "fifth") mult = INT5;
        if (mult < 0)
            mult = 2;
        if (mult > 10)
    ...
    case 3:
        ...
        break;
    default:
        THROW_NUM_ERROR(ARG_COUNT, "three to five");
    }
