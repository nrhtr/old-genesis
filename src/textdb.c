...
INTERNAL Int add_objname(char *str, Long objnum)
{
    ...
    if (lookup_retrieve_name(id, &num) && num != objnum) {
        WARN(("Attempt to rebind existing objname $%s (#%li)", str, (long) num));
        ident_discard(id);
        return 0;
    }
...
{
    holder_t *holder = holders, *old = NULL;
    Long objnum;
    Obj *obj;
    Ident id;
...
void verify_native_methods(void)
{
    ...
    for (x = 0; x < NATIVE_LAST; x++) {
        ...
        if (method == NULL) {
            ...
            if (!(method->m_flags & MF_NATIVE) && use_natives != FORCE_NATIVES) {
                if (print_warn)
                    fformat(stdout,
                            "\rWARNING: method definition %O.%s() overrides native method.\n",
...
INTERNAL void handle_parcmd(char *s, Int new)
{
    ...
    if (!cur_obj)
        DIEf("Attempt to %s parent when no object is defined.", new ? "add" : "del");
...
INTERNAL void handle_evalcmd(FILE * fp, char *s, Int new, Int access)
{
    ...
    /* die if its invalid */
    if (!method)
        DIE("Eval method definition failed");
...
INTERNAL void frob_n_print_errstr(char *err, char *name, cObjnum objnum)
{
    ...
    str = format("\rLine %l: [line %d in %O.%s()]: %s\n", method_start + line, line, objnum, name, err);
...
void compile_cdc_file(FILE * fp)
{
    ...
    fputs("\rCleaning up name holders...", stderr);
    fflush(stdout);
    cleanup_holders();
    fputs("done.\n", stderr);
    fflush(stdout);
...
void dump_object(Long objnum, FILE * fp, Bool objnames)
{
    ...
    /* try to handle this */
    if (obj == NULL) {
        printf("\rWARNING: NULL object pointer found, you likely used a corrupt binary db! Aborting!\n");
        //printf("WARNING: Attempting to work around.  This will probably create a\nWARNING: textdump with invalid ancestors\n"); //we don't attempt to work around it :S
        return;
    }
#if 0
    ...
    for (i = 0; i < obj->vars.size; i++) {
        ...
        if (!cache_check(var->cclass))
            continue;
        str = data_to_literal(&var->val, ((objnames ? DF_WITH_OBJNAMES : 0) | DF_INV_OBJNUMS));
        fputs("var ", fp);
    ...
    for (i = 0; i < obj->methods.size; i++) {
        ...
        if (meth->m_flags & MF_NATIVE && meth->native != -1) {
            if (strcmp(ident_name(meth->name), natives[meth->native].name))
                fprintf(fp, "bind_native .%s() .%s();\n\n", natives[meth->native].name, ident_name(meth->name));
        }
    }
