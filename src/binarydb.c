...
{
    Bool isdirty = YES;
    char system[LINE], v_major[LINE], v_minor[LINE], v_patch[LINE], magicmod[LINE], search[LINE];
    char *s;
    FILE *fp;

    v_major[0] = v_minor[0] = v_patch[0] = magicmod[0] = system[0] = search[0] = (char) NULL;
    ...
    if (isdirty) {
        fprintf(stderr, "** Binary database \"%s\" is incompatible, systems:\n\
                         ...
                         ** this: <%s> %d.%d-%d (module key %li)\n", c_dir_binary, system, atoi(v_major), atoi(v_minor), atoi(v_patch), atol(magicmod), SYSTEM_TYPE, VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, (long) MAGIC_MODNUMBER);
...
{
    struct stat statbuf;
    char fdb_objects[LINE], fdb_index[LINE];
    off_t offset;
    Int size;
    Long objnum;
    ...
    if (lookup_retrieve_objnum(objnum, &old_offset, &old_size)) {
        if ((tmp1 = NEEDED(new_size, BLOCK_SIZE)) > (tmp2 = NEEDED(old_size, BLOCK_SIZE))) {
            ...
            if (check_free_blocks(tmp1 - tmp2, LOGICAL_BLOCK(old_offset) + tmp2)) {
                ...
                db_mark(LOGICAL_BLOCK(old_offset) + tmp2, BLOCK_SIZE * (tmp1 - tmp2));
            ...
            if (tmp1 < tmp2) {
                db_unmark(LOGICAL_BLOCK(old_offset) + tmp1, BLOCK_SIZE * (tmp2 - tmp1));
            }
            new_offset = old_offset;
        }
    } else {
...
{
    Obj *obj = cache_retrieve(objnum), *obj2;
    Long other;
