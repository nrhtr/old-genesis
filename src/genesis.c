...
INTERNAL void initialize(Int argc, char **argv)
{
    ...
    cStr *str;
    cData arg;
    char *opt = NULL, *name = NULL, *basedir = NULL, *buf = NULL;
    FILE *fp;
    Bool dofork = YES;
    pid_t pid;
            ...
            case 'g':
                {
                    ...
                    if (buf[0] == '#') {
                        ...
                        usage(name);
                        fprintf(stderr, "** invalid group name -g: '%s'\n", buf);
                        exit(1);
                    } else {
                        gid = (gid_t) gr->gr_gid;
                    }
            ...
            case 'u':
                {
                    ...
                    if (buf[0] == '#') {
                        ...
                        usage(name);
                        fprintf(stderr, "** invalid user name -u: '%s'\n", buf);
                        exit(1);
                    } else {
                        uid = (uid_t) pw->pw_uid;
                    }
    ...
    if (strccmp(c_logfile, "stderr") == 0)
        logfile = stderr;
    else if (strccmp(c_logfile, "stdout") != 0 && (logfile = fopen(c_logfile, "ab")) == NULL) {
    ...
    if (strccmp(c_errfile, "stdout") == 0)
        logfile = stdout;
    else if (strccmp(c_errfile, "stderr") != 0 && (errfile = fopen(c_errfile, "ab")) == NULL) {
...
#undef DIE
    ...
    if (dofork) {
        ...
        if (pid != 0) {
            ...
            else if (waitpid(pid, &ignore, WNOHANG)
                     == F_FAILURE)
...
#define GETTIME() gettimeofday(&tp, NULL)
    ...
    while (running) {
        ...
        if (heartbeat_freq != -1) {
            ...
            GETTIME();
            seconds = (preempted ? 0 : ((SECS >= next) ? 0 : next - SECS));
        } else {
            /*
...
Options:\n\n\
    ...
    -p <port>   prebind port, can exist multiple times.  Port must be\n\
                ...
                be an IP address.  All below are valid:\n\n\
                    206.81.134.103:80\n\
                    :-20\n\
                    :23\n\n", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, name, c_dir_binary, c_dir_root, c_dir_bin, c_logfile, c_errfile, c_runfile, cache_width, cache_depth);
}
