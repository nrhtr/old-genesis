...
cStr *string_add(cStr * str1, cStr * str2)
{
    ...
    MEMCPY(str1->s + str1->start + str1->len - str2->len, str2->s + str2->start, str2->len);
...
cStr *string_parse(char **sptr, char delim)
{
    cStr *str;
    char *p, *s, *start = *sptr;
    ...
    for (p = s = start; *p && (*p != delim); p++, s++) {
        if (*p == '\\' && *(p + 1) && (*(p + 1) == delim || *(p + 1) == '\\'))
            p++;
        *s = *p;
    }
