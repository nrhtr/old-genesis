...
Int data_cmp(cData * d1, cData * d2)
{
    ...
    case STRING:
        return strccmp(string_chars(d1->u.str), string_chars(d2->u.str));       /* NOTE: can we change this to case-sensitive strcmp? */
...
/* FIXME: this is really buggy and crashy... halp */
char *dict_from_literal(cData * d, char *s)
{
    while (isspace(*s))
        s++;
    ...
    //make the list that will hold our set of associations
    cList *assocs = list_new(0);
    cData pair;

    d->type = -1;

    while (*s && *s != ']') {
        pair.u.list = list_new(0);
        pair.type = LIST;
        ...
        pair.u.list = list_add(pair.u.list, d);
        data_discard(d);        //we free the 2nd allocation to d at the end of the loop along with the whole pair list
